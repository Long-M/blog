...
public class QQLoginFilter extends AbstractAuthenticationProcessingFilter {
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException, ServletException {
        ...
        if (StringUtils.isBlank(code)) {
            logger.info("没有获取到code的值,引导用户到QQ授权页面");
            response.sendRedirect(String.format(GET_AUTHORIZATION_CODE, CODE, CLIENT_ID, REDIRECT_URI, UUID.randomUUID().toString(), SCOPE));
            return null;
        }
        logger.info("当前是从QQ授权登录返回,获取到code的值为:{}", code);
        logger.info("根据code:{}的值取换取access_token的值", code);
        ...
        logger.info("根据code:{}获取access_token的返回值是:{}", code, result);
        ...
        logger.info("根据accessToken去获取用户的openId的值。");
        result = restTemplate.getForObject(String.format(GET_OPEN_ID, accessToken), String.class);
        logger.info("根据accessToken：{}换取openId的结果为:{}", accessToken, result);
        String openId = StringUtils.substringBetween(result, "\"openid\":\"", "\"}");
        logger.info("根据accessToken:{}换取的openId的值为:{}", accessToken, openId);
